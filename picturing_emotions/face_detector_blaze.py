...
class Blaze_Detect:
    ...
    def plot_detections(self, with_keypoints=True):
        ...
        if isinstance(self.detections, torch.Tensor):
            self.detections = self.detections.cpu().numpy()

        if self.detections.ndim == 1:
            self.detections = np.expand_dims(self.detections, axis=0)

        print("Found %d faces" % self.detections.shape[0])
            
        for i in range(self.detections.shape[0]):
            ymin = self.detections[i, 0] * img.shape[0]
            xmin = self.detections[i, 1] * img.shape[1]
            ymax = self.detections[i, 2] * img.shape[0]
            xmax = self.detections[i, 3] * img.shape[1]

            rect = patches.Rectangle((xmin, ymin), xmax - xmin, ymax - ymin,
                                    linewidth=1, edgecolor="r", facecolor="none", 
                                    alpha=self.detections[i, 16])
            ax.add_patch(rect)

            if with_keypoints:
                for k in range(6):
                    kp_x = self.detections[i, 4 + k*2    ] * img.shape[1]
                    kp_y = self.detections[i, 4 + k*2 + 1] * img.shape[0]
    ...
    def face_crop(self):
        ...
        for i in range(self.detections.shape[0]):
            ymin = int(self.detections[i, 0] * img.shape[0])
            xmin = int(self.detections[i, 1] * img.shape[1])
            ymax = int(self.detections[i, 2] * img.shape[0])
            xmax = int(self.detections[i, 3] * img.shape[1])

            faces.append(img[ymin:ymax, xmin:xmax])
            X1.append(xmin)
            X2.append(xmax)
            Y1.append(ymin)
            Y2.append(ymax)
